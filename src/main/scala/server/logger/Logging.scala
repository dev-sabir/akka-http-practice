...
trait Logging {
  ...
  protected implicit val logger: Logger = LoggerFactory.getLogger(this.getClass())
  ...
  protected def trace(message: String, exception: Throwable): Unit = logger.trace(message, exception)
